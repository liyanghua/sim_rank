...
OUTPUT_NAME=sim_rank_step4
OUTPUT=${HPATH}/${OUTPUT_NAME}/$bizdate/
INPUT1=${HPATH}/sim_rank_sim/$bizdate/
INPUT2=${HPATH}/sim_rank_step3/$bizdate/
INPUT3=${HPATH}/count_self_sim/$bizdate/
...
${HADOOP_CMD} jar $(dirname $(which hadoop))/../contrib/streaming/hadoop-0.19.1-streaming.jar \
           -input $INPUT1 -input $INPUT2 -input $INPUT3 -output $OUTPUT $lib_extra_str \
           -jobconf mapred.reduce.tasks=100 \
           -jobconf mapred.job.name="sim rank - step 4" \
           -mapper "sim_rank_step4_mapper" -file ${CUR_DIR}/sim_rank_step4_mapper \
           -jobconf file_flag="sim_rank_step3" \
           -reducer "sim_rank_step4_reducer" -file ${CUR_DIR}/sim_rank_step4_reducer \
